...
def train_HCLS(data_file,model_dir,batch_size, nb_epoch,N,k,m,l):
    ...
    # 根据各个参数值的不同组合在(X_train, y_train)上训练模型mj
...
def test_HCLS(data_file, model_dir, outfile='prediction.txt', fprfile='fpr_file.txt',tprfile='tpr_file.txt',
...
filename = '15_PARCLIP_IGF2BP123_hg19'
k=20
m=3
l=16
bs=256
...
def run_train_HCLS(
    ...
    if train:
        print ('model training')
        train_HCLS(data_file, model_dir, batch_size=batch_size, nb_epoch=n_epochs,N=N,k=k,m=m,l=l)

def run_test_HCLS(
    data_file="./RBPdata1201/%s/test/%s/sequence.fa.gz"%(filename,number), predict=True):
    out_file = 'model/%s/prediction.txt'%(filename)
    model_dir = 'model/%s'%(filename)
    fpr_file = 'model/%s/fpr.txt'%(filename)
    tpr_file = 'model/%s/tpr.txt'%(filename)
    metricsfile = 'model/%s/metrics_file.txt' % (filename)
    ...
    if predict:
        ...
        test_HCLS(data_file, model_dir, outfile=out_file, fprfile=fpr_file, tprfile=tpr_file, metricsfile=metricsfile,onlytest=True)
